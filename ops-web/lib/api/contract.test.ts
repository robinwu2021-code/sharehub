...
const API_METHODS: Record<string, readonly string[]> = {
  ...
  org: ["listEmployees", "listRoles", "listAudits", "listDepartments", "listStaffPerformance", "saveDepartment", "saveRoleRow", "saveEmployee", "getDataScope", "saveDataScope", "listAllMenus", "listPermissions", "listRolePermissions", "saveRolePermissions", "getAuditDetail", "archiveRole", "unarchiveRole"],
...
describe("域切片划分", () => {
  ...
  it("方法总数仍为 311（新增/删除 API 时须自觉更新此数）", () => {
    ...
    // 否则推送发出后永远卡在「发送中」。
    expect(ALL_METHODS.length).toBe(311);
  });
});
