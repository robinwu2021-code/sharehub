...
describe("站点状态机", () => {
  ...
  it("撤场：营业中或暂停中都可以，原因必填", () => {
    cur().status = "ACTIVE";
    expect(ss.withdrawSite(no, "场地方收回").status).toBe("WITHDRAWING");
    cur().status = "PAUSED";
    expect(ss.withdrawSite(no, "再来一次").status).toBe("WITHDRAWING");
    cur().status = "ACTIVE";
    expect(() => ss.withdrawSite(no, "")).toThrowError(/原因|reason/i);
  });
...
describe("开业清单", () => {
  it("★ 四条都真查数据——空站点应该全不通过（写死 true 的清单永远全绿）", () => {
    const c = ss.siteOpeningChecklist(no);
    expect(c.allPassed).toBe(false);
    expect(c.items.map((i) => i.key).sort()).toEqual(["contract", "openHours", "owner", "point"]);
