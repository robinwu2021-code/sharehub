...
// 并且 ALL/SELF 会清空 scopeValues（否则残留脏 refs 会被后端 DataScopeHandler 误用）。
...
describe("ALL / SELF 清空 scopeValues", () => {
  it("原本有范围值的角色改成 ALL → scopeValues 清空", () => {
    saveRoleDataScope("OPS", "REGION", "AE-DU,AE-AZ");
    expect(find("OPS").scopeValues).toBe("AE-DU,AE-AZ");
    saveRoleDataScope("OPS", "ALL", "AE-DU,AE-AZ"); // 即使调用方仍传了值
    expect(find("OPS").scopeValues).toBe("");
  });

  it("改成 SELF → scopeValues 清空", () => {
    // 用 CS 而非 AGENT：AGENT 角色的数据范围被服务端守卫锁死（见下方越权守卫用例）
    saveRoleDataScope("CS", "SELF", "AG001,AG002");
    expect(find("CS")).toMatchObject({ dataScope: "SELF", scopeValues: "" });
  });
});
...
describe("CSV 归一", () => {
  ...
  it("saveRoleDataScope 落库前也走归一", () => {
    ...
    expect(find("CS").scopeValues).toBe("AE-DU,AE-SH");
  });
});

describe("scopeValues 引用真实主数据（配合 integrity.test.ts，CSV 字段它查不到）", () => {
  ...
  it("每个角色种子数据里的范围 ID 都能在对应主数据里找到", () => {
    ...
    for (const r of snapshot) {
      const values = (r.scopeValues ?? "").split(",").map((s) => s.trim()).filter(Boolean);
      if (r.dataScope === "ALL" || r.dataScope === "SELF") {
        if (values.length) bad.push(`roles[${r.code}] dataScope=${r.dataScope} 却带了范围值 "${r.scopeValues}"`);
        continue;
      }
      ...
      for (const v of values) {
        if (!(pool as Set<string>).has(v)) bad.push(`roles[${r.code}].scopeValues 的 "${v}" 不存在于 ${to}`);
      }
    }
    expect(bad, `\n${bad.join("\n")}\n`).toEqual([]);
  });
