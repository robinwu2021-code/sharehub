...
export default function NoticesPage() {
  ...
  // 导出列与表格可见列一致（TDD §10.2）。并入本页前，营销管理那份是有导出的，不能丢。
  const onExport = () => exportCsv<Notice>("公告管理", [
    { header: "公告号", value: (n) => n.noticeNo },
    { header: "标题（中）", value: (n) => n.title },
    { header: "类型", value: (n) => TYPE[n.type].label },
    { header: "置顶", value: (n) => (n.pinned ? "是" : "否") },
    { header: `生效期（${zone}）`, value: (n) => `${formatMarketTime(n.startAt)} ~ ${n.endAt ? formatMarketTime(n.endAt) : "长期"}` },
    { header: "状态", value: (n) => VIEW[noticeView(n, now)].label },
  ...
  return (
    <div>
      ...
      <Toolbar search={keyword} onSearch={setKeyword} searchPlaceholder="搜索标题 / 编号 / 发布人" onExport={rows.length ? onExport : undefined} onAdd={openNew} addLabel="新建公告" canAdd={canWrite}>
