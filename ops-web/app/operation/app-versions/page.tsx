...
import { currentReleased, grayRelease, validateAppVersion } from "@/lib/rules/operation-rules";
...
export default function AppVersionsPage() {
  ...
  const onExport = () => exportCsv<AppVersion>("应用版本", [
    { header: "平台", value: () => PLATFORMS.find((p) => p.key === platform)?.label ?? platform },
    { header: "版本号", value: (v) => v.versionNo },
    { header: "构建号", value: (v) => v.buildNo },
    { header: "更新说明（中）", value: (v) => v.releaseNote },
    { header: "强制更新", value: (v) => (v.forceUpdate ? `是 · 最低 ${v.minSupported}` : "否") },
    { header: "灰度（%）", value: (v) => v.rolloutPercent },
    { header: "状态", value: (v) => STATUS[v.status].label },
    { header: "发布时间", value: (v) => (v.releasedAt ? formatMarketTime(v.releasedAt) : "") },
  ], rows);
  ...
  return (
    <div>
      ...
      <Toolbar search={keyword} onSearch={setKeyword} searchPlaceholder="搜索版本号 / 更新说明" onExport={rows.length ? onExport : undefined} onAdd={openNew} addLabel="新建版本" canAdd={canWrite} />
      <DataTable
