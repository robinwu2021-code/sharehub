  ...
  return (
    <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
      ...
      <div className="flex items-center gap-2">
        <button className="rounded-lg bg-secondary px-2.5 py-1 transition-colors hover:bg-accent disabled:opacity-40 disabled:hover:bg-secondary" disabled={page <= 1} onClick={() => onPage(page - 1)}>{t("common.prevPage")}</button>
        <span className="tabular-nums">{t("common.pageOf", { p: page, total: pages })}</span>
        <button className="rounded-lg bg-secondary px-2.5 py-1 transition-colors hover:bg-accent disabled:opacity-40 disabled:hover:bg-secondary" disabled={page >= pages} onClick={() => onPage(page + 1)}>{t("common.nextPage")}</button>
      </div>
    </div>
  );
}
