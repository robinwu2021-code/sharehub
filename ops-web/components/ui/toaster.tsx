...
export function Toaster() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-5 z-[100] flex flex-col gap-2" style={{ insetInlineEnd: "1.25rem" }}>
      {toasts.map((t) => {
        ...
        return (
          <div
            key={t.id}
            className={cn(
              "pointer-events-auto flex items-center gap-2.5 rounded-xl px-4 py-2.5 text-sm shadow-[var(--card-shadow)]",
              TONE[t.type],
            )}
          >
            <Icon className="size-4 shrink-0" />
            <span className="max-w-[320px]">{t.message}</span>
            <button onClick={() => dismiss(t.id)} className="ml-1 opacity-60 hover:opacity-100" aria-label="关闭">
