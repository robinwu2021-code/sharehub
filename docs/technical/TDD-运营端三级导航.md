...
**文档状态**：已实现（2026-07-12）
> ## ⚠️ 部分内容已过时（2026-07-30）
>
> 本文写于三级导航初建期，以下设计**已变更**，实施请以
> [设计规范-布局与外壳.md](设计规范-布局与外壳.md) 为准：
>
> | 本文写的 | 现状 |
> |---|---|
> | `navMode: 'panel' \| 'miller'` 双模式可切换 | **miller 已删除**。切换开关是元设置，占着面板头部最显眼位置，运营用户不需要在工作中切换导航形态。`NavMode` / `NAV_MODE_DEFAULT` / `MILLER_*_WIDTH` 均已从 `lib/nav.ts` 移除 |
> | 7 个域（L1）| **15 个一级模块**。域层已整层删除，模块提上来当 L1，叶子的 `group` 顺位成 L2（四层压成三层）|
> | `PANEL_WIDTH` ~208px | **176px** |
>
> 其余部分（RBAC 过滤、深链高亮、面包屑、persist key）仍然有效。
