...
## 17.2 B5-财务组实施决策回灌（2026-07-29）

> 实现 §5 分润统计 / §6 充值订单 时规格未覆盖、由实现者判断的点。**已确认，后续批次与后端实现须遵守**。

| # | 决策 | 结论 |
|---|---|---|
| 1 | 两节都没写**币种** | 补 `currency`（AED）。**规格通病**：凡涉及金额的类型都必须带 `currency`，页面一律 `money()` 格式化 |
| 2 | `RechargeOrder` 只有 `paidAt`，但 PENDING/FAILED 单没有 | 新增 `createdAt`，**日期范围一律按下单时间筛选**（按 paidAt 筛会把未支付单全漏掉）。列表仍展示 paidAt |
| 3 | `ShareSummary` 无自然主键 | `dimension + payeeNo + period` 作 rowKey；`payeeNo` 单独成列便于与 VEN3xx/AG00x 对齐 |
| 4 | ⚠️ **代理商编号实际是 `AG00x` 不是 `AGT00x`** | 按"mock 跨页自洽"原则取现有 `db.agents` 的真实值。**规格与任务书里写的 AGT00x 是错的**，后续批次注意 |
| 5 | 分润统计的排序放在数据层 | `listShareSummaries` 支持 `sortKey/sortDir`，前端只受控。**后端 `/api/trade/share-summaries` 必须支持这两个 query 参数**，否则翻页后排序不一致 |
| 6 | 行深链 `?tab=records&payee=xxx` 的参数消费 | 分润明细 tab **已真正消费该参数**（落成搜索词），不静默丢弃。**规范**：深链带的参数，目标页必须消费或显式忽略并说明 |
| 7 | 充值订单也加了导出（规格只要求分润统计有）| 保留。资金流水页导出属 G2 通用能力，**凡流水类页面一律给导出** |
| 8 | 端点命名 | `/api/trade/share-summaries`（trade 域聚合）· `/api/user/recharge-orders`（归 user 域，与权限码 `user:wallet:read` 一致）。待同步 [api/README](../api/README.md) |
...
## 17.3 B2/B3/B4 实施决策回灌（2026-07-29）

| # | 决策 | 结论 |
|---|---|---|
| 1 | `NotifyBlacklist.channel` 补 `WHATSAPP` | 采纳。§9 已是全渠道，拉黑少一个渠道等于该渠道不可拦截，与「全渠道拉黑」结论自相矛盾 |
| 2 | 新增业务键 `NotifyBlacklist.blockNo`、`AppVersion.versionId`(平台-版本号) | 采纳。复合键不便做 rowKey/编辑；`versionNo` 跨平台会重复 |
| 3 | 补 `countryName`（登录设置/税率）、`AppVersion.releasedAt` | 采纳。列表只显国家码不可读 |
| 4 | 额外校验：登录设置**至少保留一种登录方式**；强更时要求灰度 100%；计费单位 > 0 | 采纳。强更+灰度<100% 会把部分用户永久拦在旧版**无法升级**，属危险配置 |
| 5 | 「重发」按钮**本批次不做** | 采纳。重发会二次扣费且需幂等键，契约无对应能力 → **先定契约再做**，本批次只做失败原因详情 |
| 6 | 手动拉黑的 target 不在前端脱敏 | 采纳。脱敏是落库侧职责，页面不应依赖 mock 层实现 |
| 7 | ⚠️ **B0 遗漏：`DataTable` 无行级样式** | 三处「整行高亮/灰显」被迫降级为单元格级 → **已补 `rowClassName?: (row)=>string`**，并回改预约超时/白名单过期/套餐下架为真·整行 |
| 8 | ⚠️ **进度条被手搓 3 次** | 已抽出 `components/ui/progress.tsx`（支持 `warnAt` 预警变色、`showText`），三处合并 |
| 9 | ⚠️ **新权限码未进角色映射** | 已按 [功能权限清单 §13](../requirements/功能权限清单.md) 补 `lib/permissions.ts`：OPS(发送记录/版本/参数/市场)、CS(发送记录/拉黑/问题)、FINANCE(支付渠道/业务规则/银行/税率) |
| 10 | ⚠️ **系统设置 10 个叶子无 `perm`** | 无 perm 的叶子跟随父模块 `canModule` → 给 FINANCE 任一 `system:*` 码就会让它看到「登录设置/系统参数」等运维项。**已给全部叶子补显式 perm**，并加单测锁死「系统设置每个叶子都必须有 perm」 |
| 11 | 业务规则的手续费口径 | 页面已注明「提现审核页手续费口径的唯一来源」，但**提现审核页仍硬编码 0.6%**，联动改造待做（跨页，记入遗留） |
