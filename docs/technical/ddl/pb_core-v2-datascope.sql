...
ALTER TABLE loc_location
  ADD COLUMN agent_no VARCHAR(36) NULL COMMENT '归属代理(冗余·随站点，数据范围锚点)' AFTER site_name;

ALTER TABLE dev_cabinet
  ADD COLUMN site_no  VARCHAR(36) NULL COMMENT '归属站点(冗余·随点位，数据范围锚点)' AFTER location_no,
  ADD COLUMN agent_no VARCHAR(36) NULL COMMENT '归属代理(冗余·随站点，数据范围锚点)' AFTER site_no;

ALTER TABLE ord_rent
  ADD COLUMN location_no VARCHAR(36) NULL COMMENT '借出点位' AFTER cabinet_no,
  ADD COLUMN site_no     VARCHAR(36) NULL COMMENT '借出站点(冗余·数据范围锚点)' AFTER location_no,
  ADD COLUMN agent_no    VARCHAR(36) NULL COMMENT '归属代理(冗余·数据范围锚点)' AFTER site_no;

ALTER TABLE wo_order
  ADD COLUMN location_no VARCHAR(36) NULL COMMENT '点位' AFTER cabinet_no,
  ADD COLUMN site_no     VARCHAR(36) NULL COMMENT '站点(冗余·数据范围锚点)' AFTER location_no,
  ADD COLUMN agent_no    VARCHAR(36) NULL COMMENT '归属代理(冗余·数据范围锚点)' AFTER site_no;
...
-- 数据范围过滤 + 列表默认排序一次走完
ALTER TABLE ord_rent    ADD KEY idx_ord_scope  (tenant_id, agent_no, status, created_at);
ALTER TABLE wo_order    ADD KEY idx_wo_scope   (tenant_id, agent_no, status, created_at);
ALTER TABLE dev_cabinet ADD KEY idx_cab_scope  (tenant_id, agent_no, status);
ALTER TABLE loc_location ADD KEY idx_loc_scope (tenant_id, agent_no);
