...
| # | 任务 | 判据 | 依赖 |
|---|---|---|---|
| T1-1 | 抽 `AbstractTokenAuthFilter`，两个过滤器改为继承 | 两端行为不变；新增失败态只需改基类 | — |
| T1-2 | 令牌前缀硬校验（`stk_`/`atk_`/`ctk_`） | 拿 C 端令牌打 `/api/**` → 401 且**不查库** | T1-1 |
| T1-3 | `clientCode` 落进审计（OPS / AGENT / MP） | 审计里分得出代理自己做的与运营做的 | T1-1 · T3-1 |
| T1-4 | **卡口**：`/mp/**` 端点不得从请求体/参数取用户标识 | 反向对照：加一个接受 `userNo` 入参的探针 → 红 | — |

## T2 · 运行日志（2–3 天）

| # | 任务 | 判据 |
|---|---|---|
| T2-1 | `logback-spring.xml`：三目录、按天+200MB 切、MDC traceId、`total-size-cap` | 本地起服后 `/data/log/powerbank/app/*.log` 有内容且带 traceId |
| T2-2 | 五个配置项接 `application.yml`，全部带默认值 | 改环境变量能生效，不改代码 |
| T2-3 | `deploy/tencent/logrotate/powerbank` + 压缩 cron（5 天）+ systemd 改 `StandardOutput=append:` | 服务器上跑一轮 `logrotate -f` 正常；5 天前的 `.log.*` 被 gzip |
| T2-4 | **卡口**：禁止 `printStackTrace` / `System.out`（棘轮台账） | 反向对照：加一处 → 红 |
| T2-5 | 日志规范写进 CLAUDE.md（五条硬规矩 + 级别口径） | — |

## T3 · 操作审计（3–4 天）

| # | 任务 | 判据 |
|---|---|---|
| T3-1 | 加 `outcome` 列（SUCCESS/DENIED/FAILED），失败也记 | 403 的操作能查到 |
| T3-2 | `@Audited` 注解 + 改前改后快照（增量启用） | 分润规则改费率能查出改前改后 |
| T3-3 | 覆盖 `/internal/**`，actor 记 `SYSTEM:<service>` | 划拨回写留痕 |
| T3-4 | 审计写入失败改为**告警**而非 `log.warn` | 现在静默降级，等于审计可以悄悄失效 |
| T3-5 | 月分区 + 归档策略 | — |
