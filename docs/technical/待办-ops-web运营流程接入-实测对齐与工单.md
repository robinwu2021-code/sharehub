...
| # | 缺口 | 现在前端怎么兜的 | 后端该怎么改 |
|:-:|---|---|---|
| 1 ✅ | ~~门禁 `fixHref` 指向不存在的路由~~ | — | **2026-09-26 已修**（`2b4893a`）：后端出运营端真实路由，前端三层降级为兜底。`FixHrefRoutesTest` 静态扫字面量逐条比对页面与页签，当场又抓出第四处 `/orders?siteNo=` |
| 2 ✅ | ~~人工派单不写 `assignee_type`~~ | — | **2026-09-26 已修**（`49bcbd7`）：按受理人反查代理 / 员工写对三列；驳回三列一起清；顺带修了「受理人超 36 字符 → 派单 500」 |
| 3 ✅ | ~~零明细的调拨单可以发货~~ | — | **2026-09-26 已修**（`0cadf8a`）：⚠️ **发货有两个入口**（`…/ship` 与保存时传 `status=IN_TRANSIT`），只堵前者等于没堵，两条都补了 |
| 4 ✅ | ~~划拨候选端点返回裸数组、参数名不一致~~ | — | **2026-09-26 已修**（`ad2f834`）：两侧同批归一，前端适配层撤掉。⚠️ `total` 是**候选池（前 500 条）**里的条数、与能翻到的页数一致 —— 返回全表 count 会「总数 800 却翻到第 6 页就空」 |
| 5 ✅ | ~~`/api/agent/assignments` 是空实现~~ | — | **2026-09-26 已实现**（`ad2f834`）：按 `agent_no` 聚合站点 / 机柜，用例与库直接对账。同文件的代理绩效 `page()` 也是空实现，但属 L2 分期功能、不在本清单 |
| 6 ✅ | ~~找不到代理当前的在途清退单~~ | — | **2026-09-26 已修**（`0cadf8a`）：`GET /api/agent/agents/{agentNo}/exit`，没有在清退时返回 null 而不是 404 |
| 7 ✅ | ~~清退中的代理仍可被改回启用~~ | — | **2026-09-26 已修**（`0cadf8a`）：保存路径加闸，不碰状态的编辑照常放行 |
| 8 ✅ | ~~调整项列表缺 `period`、不支持筛选~~ | — | **2026-09-26 已修**（`3ce3635`）：出参补 `period`，列表补 kind / source / period 三个筛；枚举拼错当场 400 而不是静默返空 |
...
| 11 ✅ | ~~机柜 / 充电宝出参没有质检状态与所在仓~~ | — | **2026-09-26 已修**（`3ce3635`）：补 `qcStatus` / `warehouseNo`（实体 V107 就有，只是没进 VO）。⚠️ 仓库列表端点仍缺，调拨选仓还是手填编号 |
| 12 ✅ | ~~告警列表缺两张卡的筛选参数~~ | — | **2026-09-26 已修**（`3ce3635`）：`preset=DISPOSED_OPEN\|HEALED_TODAY`。⚠️ 关键不是「让卡片可点」，是**摘要与列表用同一段条件**，用例直接断言「卡片数字 == 筛出来的条数」 |
...
| 14 ⏸ | **站点可借率 / 可还率 / 受影响订单数没有接口** | 摘要条只用 summary 的四个数 | **本轮不做（2026-09-26 裁决）**：三个指标的口径要先定（按什么时间窗？按柜还是按站？受影响订单怎么算），口径没定就实现等于替业务拍一个会进考核的数字。单独一轮，先定口径 |

> **2026-09-26 进度**：14 条里 **13 条已修**，剩 #14（站点可借率，口径待定，已裁决本轮不做）。
> ⚠️ **这份清单同时有两个会话在推进**（#13 是另一个会话修的）——动手前先看一眼 git log 与本表，
> 别把已经修好的那条再修一遍。
...
>
> **第二批（#1 #3 #6 #7）顺带证明了「前端兜底」的代价**：#3 的空单发货有**两个入口**，
> 前端只知道自己调的那个；#1 的路由知识散在三处前端各写一遍。
> 两者都是「闸放在前端」的典型后果 —— 换个客户端（C 端、代理端、curl）就再踩一次。
>
> ⚠️ 另一个会话在途的 `AgentOpsTest#exitingAgentCannotBeReEnabled` 与 #7 同意图，
> 但它的请求体缺 `cabinetCount`/`shareRate`，而 `Agent` record 用的是原始类型 `int`/`double`，
> Jackson 对缺失的原始类型直接 400 —— **那是用例的问题，不是闸没生效**。
> 顺带暴露一个值得单独处理的问题：原始类型让「只改一个字段」的部分更新不可能，
> 调用方得连聚合值 `cabinetCount` 一起传（而服务端根本不用它）。
