...
> **5–7 拆成了 a/b 两半**：数据层（类型 / 契约 / mock / 测试）不碰争用文件，已全部完成；
> 页面 UI 必须改 `app/devices/page.tsx` · `app/alarms/page.tsx` · `app/work-orders/page.tsx`
> 与 `components/status.tsx`，仍在等。
>
> 新类型落在 **`lib/types/device-ops.ts` · `alarm-biz.ts` · `workorder-ext.ts`** 三个新文件，
> 而不是并进被占用的 `device.ts` / `alarm.ts` / `workorder.ts` ——
> 这是 CLAUDE.md 说的「有在途改动则改用非侵入方式」。
> ⚠️ **它们不是永久归宿**：三个域文件空出来后应当并回去、新文件删除。
> 每个文件头部都写了这句话，别让临时落点变成「类型分两处」的新约定。

### ⛔ 5–7 为什么停下：另一个会话正在改同一批文件

实测（`git status`，持续数小时未变）：
`lib/types/device.ts` · `lib/types/alarm.ts` · `lib/types/workorder.ts` ·
`app/devices/page.tsx` · `app/work-orders/page.tsx` · `components/status.tsx`
**全部是 `M`（另一会话在途）**。他们的代码注释带「2026-09-25 批次 C4 / D4」字样，
是另一套批次编号 —— 说明那边有自己的计划在推进，不是零散改动。

硬做下去的结果不是冲突就是重复劳动。接手前先确认这六个文件 `git status` 为空。

### 📌 欠着一笔：台账没删

`backend/known-api-align-gaps.txt` 的规矩是「接好一条就删一条，**不删也红**」。
批次 1–4 接了 21 个端点（合同 12 · 站点 7 · 文件 2），对应的 C 类条目本该删掉 ——
但**那个文件也在另一会话手里**（连同 `docs/api/contract.json`），不能动。

接手的人：动 5–7 之前先把这 21 条删掉，并跑 `npm run check:drift` 确认。
⚠️ 当前 `npm run check:drift` **七项全过**（已知缺口 129，无新增）——
它没因为「修好了却没删」而变红，八成是 `contract.json` 处于另一会话的在途状态，
**别把这个绿当成台账已经对齐**。
