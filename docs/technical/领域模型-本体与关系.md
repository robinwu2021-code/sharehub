...
> **表名后的 ⏸**：表已建，但**全代码零引用**（由 [L0 对账](./实体-领域对象对账表.md) §2.1.2 机械扫出）。
> 标出来是因为这类表最容易在下一轮重构里被当成"在用"而原样保留 —— 它究竟是
> ADR-021 的待接入预留，还是建早了，**待确认**。
...
| **运营主体 Operator** 🆕 | **谁在经营这个站点**——出钱、铺设、运维、拿分润的主体。平台自营 / 代理商 / 城市合伙人都是它的一种（[ADR-029](./ADR/ADR-029-运营主体抽象.md)） | `operator_no` | 现寄居 `agt_agent`；S1 窗口可独立为 `org_operator` | ENABLED / SUSPENDED |
| **代理商 Agent** | 运营主体的一种：运营商**体内**的分润伙伴，出钱买设备、包一片区域铺设与运维。用运营商的品牌，C 端无感知 | `agent_no` | `agt_agent` | ENABLED / SUSPENDED |
| **入驻申请 AgentApply** 🆕 | 运营主体的**入驻申请单**。商家自助注册与运营商代建**同落这一张**、同一个状态机——「条件相同」在模型上就是不按来源分叉（[ADR-030 §三](./ADR/ADR-030-账号与主体-一个账号可属多个运营主体.md)）。审核通过在一个事务里派生主体 + 属主成员关系行 | `apply_no` | *待建* `agt_apply` | DRAFT / SUBMITTED / REVIEWING / APPROVED / REJECTED |
| **自然人 Principal** 🆕 | 代理端的**登录主体**——一个人一行。持有两个登录标识（手机号、邮箱，均全局唯一）与**一套凭据**。标识按 `*_hash` 做键、`*_mask` 只显示、`*_enc` 需要还原时用（[ADR-030 §2.3](./ADR/ADR-030-账号与主体-一个账号可属多个运营主体.md)） | `principal_no` | *待建* `agt_principal` | ACTIVE / DISABLED |
| **代理账户 AgentAccount** | **「人 × 运营主体」的成员关系行**，不是登录主体本身。一个人在 N 个主体下就是 N 行，挂 `principal_no`；`is_owner` 属主走旁路，`is_primary` 定默认主体 | `account_no` | `agt_account` | ACTIVE / DISABLED |
| **代理辖域 AgentRegion** | 代理商可经营的区域集合 | (`agent_no`,`region_id`) | `agt_agent_region` | — |
| **划拨流水 Assignment** | 设备 / 点位 / 站点划给谁、何时收回的**流水**（append，只插不改）。现状权威在 `dev_cabinet.agent_no` / `loc_location.agent_no` 的冗余列（ADR-012）；两者不可互相替代——用流水推现状要全表回放，用现状查历史什么都查不到 | `assign_no` | `agt_assignment` | action: ASSIGN / REVOKE |
...
| **订单 Order** | 消费者对一台设备的一次完整使用。通用主表 + 按形态的扩展表（§三） | `order_no` | `ord_order` + `ord_rent_ext` · `ord_charge_ext` · `ord_locker_ext` | CREATED → ACTIVE → ENDED → SETTLED → CLOSED · EXCEPTION |
...
| 对象 | 定义 | 标识 | 表 | 状态 |
|---|---|---|---|---|
| **告警 Alarm** | 设备上报的异常事实，归一化为告警码 + 厂商码 | `alarm_no` | `dev_alarm` | OPEN / ACKED / CLOSED |
...
AgentApply ──审核通过(事务)──▶ Agent + Principal(命中则复用) + AgentAccount(属主)
Principal ──成员关系(N:M)──▶ Agent      // 载体就是 AgentAccount
...
erDiagram
  ...
  BRAND ||--o{ SITE : "以此品牌运营"
  AGENT ||--o{ SITE : "经营(agent_no；直营=PLATFORM)"
  VENUE ||--o{ SITE : "提供场地"
  VENUE ||--o{ CONTRACT : "签约"
...
**所以站点→代理商建模为关联而非归属**，我方 `agt_assignment(action=ASSIGN/REVOKE)` 已经是这个形状。
...
#### 2.4.1 账号 → 主体 → 商户 / 站点：一条链，**三段形状都不一样**

2026-09-23 用户表述：「**一个账号对应多个主体，每个主体多个商户，多个站点。**」
这条链成立，但**三段的形状各不相同，按一棵树去实现会错三处**：

```
自然人 Principal（手机号 + 邮箱，两个标识一套凭据）
   │  ① N : M —— 不是一行多主体，是同一个人的多行（agt_account 成员关系）
   ▼
运营主体 Operator（AG001 / AG002 …）
   │
   ├──② 1 : N ──▶ 商户 Merchant   pay_merchant  唯一键 (operator_no, channel, market, scope_level, scope_ref)
   │                              ——「多个」的维度是 通道 × 市场 × 适用范围
   │
   └──③ 1 : N ──▶ 站点 Site       loc_site.operator_no（主责，NOT NULL）
                        │
                        ├── ④ 站点 ↔ 商户：**不绑定**，按 scope 逐级解析（SITE → DEVICE_TYPE → ALL）
                        ├── ⑤ 站点 M : N 主体：loc_site_operator 责任行（分钱依据，与 ③ 不是一回事）
                        └── ⑥ 站点 ── venue_no ──▶ 场地方（第二个正交归属，见 §2.2）
```

| 段 | 形状 | 读错会怎样 |
|:-:|---|---|
| ① 账号 → 主体 | **N : M**（[ADR-030](./ADR/ADR-030-账号与主体-一个账号可属多个运营主体.md)） | 写成「一行挂 `operator_nos` 数组」→ 撤销单个成员关系要读-改-写，并发下互相覆盖且不报错 |
| ② 主体 → 商户 | 1 : N，**但 N 的维度是 通道 × 市场 × scope** | 写成「一个主体一个商户号」→ 一个渠道多商户号放不下（ai-shop V14 为此改过唯一键，是高风险操作） |
| ③ 主体 → 站点 | 1 : N（**主责**） | 与 ⑤ 的责任行混为一谈 → 「出资方」自动获得运维操作权，或换个运维方就改变数据范围 |
| ④ 站点 → 商户 | **不是一条边** | 给 `loc_site` 加 `pay_merchant_no` 外键 → 与 scope 解析双写打架，且表达不了「按设备类型分账」 |

> **②③ 是从主体分出的两条并列 1:N，不是「主体 → 商户 → 站点」的三级树。**
> 一笔订单的钱进哪个商户号，是**在站点上按 scope 解析出来的**（`SITE → DEVICE_TYPE → ALL`，取最先命中，
> 一个都没命中就抛异常），不是顺着外键找出来的。这正是 V51 明确否决 `loc_site.pay_merchant_no` 的原因。

#### 2.4.2 「一站一主体」与「一站多主体」不矛盾——它们回答不同的问题

| | `loc_site.operator_no` | `loc_site_operator` 责任行 |
|---|---|---|
| 基数 | **1**（NOT NULL） | **M : N**（一行一责任） |
| 回答 | 这个站点**归谁经营**——数据范围锚点、订单落哪个主体 | 谁出资 / 谁拓展 / 谁运维 / 谁管效果——**凭什么分钱** |
| 用在哪 | `WHERE operator_no IN (...)`；订单与设备的 `operator_no` 快照 | `ShareGenerator` 展开成多条 `share_record` |

**新增不变量（两份 ADR 此前都没钉住，实现时一定会摇摆）**：

> **`loc_site.operator_no` 必须在该站点的责任行中出现，且持有 `OPERATE`。**

不钉这一条，就会出现「数据范围归 A，而 A 在这个站点一条责任都没有」的状态——
看得见数据、分不到钱，且没有任何地方报错。
**一笔订单的 `operator_no` 写主责**（③ 的快照），分钱靠责任行展开；两者不得互相替代。
...
| 运营主体 **N — M** 账号 | ~~代理商 1—N 代理账户~~ 已按 [ADR-030](./ADR/ADR-030-账号与主体-一个账号可属多个运营主体.md) 修正：`agt_account` 是**成员关系行**，唯一键 `(operator_no, login_phone)`。每个主体**至少一行 `is_owner=1`**；同一手机号**至多一行 `is_primary=1`** |
| 代理商 1 — N 站点 | 现状 `agent_no` 可空（空 = 运营商直营）；[ADR-029](./ADR/ADR-029-运营主体抽象.md) 拟改 **NOT NULL** + 哨兵 `PLATFORM` —— NULL 进不了唯一键，平台收入也记不进分润表 |
...
| 运营主体 1 — N 商户 | 维度是 通道 × 市场 × scope；**没有商户的主体照样是主体**（未进件的新代理商） |
| 站点 1 — 1 主责主体 · M — N 责任主体 | 两者并存且必须自洽，见 §2.4.2 的新增不变量 |
| 站点 — 商户 | **无边**；按 scope 解析（§2.4.1 ④） |
| 入驻申请 1 — 0..1 主体 | 审核通过才派生；同一手机号**至多一张在途申请**（生成列 `active_key`，不用可空唯一键） |
...
### 3.3 运营主体与分润受益方：两层泛化（[ADR-029](./ADR/ADR-029-运营主体抽象.md)）

**上层 —— 谁在经营**：`代理商` 不是一个主体类型的名字，它是「运营主体」的一种。

```
Operator 运营主体（operatorNo + operatorType）
  ├── PLATFORM      平台自营（恒一行，不可停用；今天用 agent_no IS NULL 表示）
  ├── AGENT         代理商
  └── CITY_PARTNER  城市合伙人
```

**收款能力挂在主体上，不是主体的类型**（[ADR-029 §二](./ADR/ADR-029-运营主体抽象.md)，同 Stripe Connect v2 的可组合配置）：

```
Operator ─┬─ Merchant 商户（1:N）  收单：通道 × 市场 × 进件 × 子商户号   ← 只有运营主体需要
          └─ PayoutAccount 收款账户  打款：银行 + IBAN + 户名
Venue    ─── PayoutAccount 收款账户  ← 场地方从不收消费者的钱，只被打款，故不配商户
```

**下层 —— 谁拿钱**：运营主体是受益方的一种**来源**，场地方是另一种。两者不是同一个枚举。

```
Payee 分润受益方（payeeType + payeeNo）
  ├── OPERATOR  运营主体 ── 费率来源：分润规则 + 责任 basis（ADR-027）
  └── VENUE     场地方   ── 费率来源：进场合同
```

> 场地方是受益方但**不是**运营主体（不经营、不出资、只提供场地）；
> 平台既是运营主体也是受益方。把两者压成一个枚举，就会出现「场地方的 operatorType 是什么」这种没有答案的问题。
...
Principal（realm + subjectNo + 凭据 + 会话）
  ...
  ├── AGENT     代理账户  atk_  → 角色 + 强制 operator_no（N:M，见下）
...
> **AGENT 这一支是 N:M**（[ADR-030](./ADR/ADR-030-账号与主体-一个账号可属多个运营主体.md)）：一个自然人（手机号）可属多个运营主体。
> **「一个账号多主体」不是一行多主体，是同一手机号的多行**——看上去像一个账号，实现上是成员关系表。
> 会话携带 `memberships[]` + `currentOperatorNo`（请求头 `X-Operator-No` 切换，**必须校验落在成员集合内**）。
> 另注意三层不可混：主体对站点的**责任**（`loc_site_operator`）· 账号对站点的**授权**（`agt_account_site`，L2）· **数据范围**（SQL 层）。
...
| 7 | 「商户」列为禁用词 | 术语裁定 |
| 11 | 新增**自然人 Principal**：登录主体独立成表，持手机号 + 邮箱两个标识与一套凭据；代理账户降为纯成员关系行 | [ADR-030 §2.2](./ADR/ADR-030-账号与主体-一个账号可属多个运营主体.md)（2026-09-23 用户定「注册信息包含手机号码、邮件」） |
| 10 | 新增**入驻申请 AgentApply**：自助注册与代建两条入口同表同状态机；审核通过派生主体+属主账号 | [ADR-030 §三](./ADR/ADR-030-账号与主体-一个账号可属多个运营主体.md)（2026-09-23 用户定「条件相同」） |
| 9 | 代理账户重定义为**成员关系**（人 × 主体，N:M）；账号对站点的**授权**与主体对站点的**责任**明确分开 | [ADR-030](./ADR/ADR-030-账号与主体-一个账号可属多个运营主体.md) |
| 8 | 新增**运营主体 Operator** 抽象（§3.3 上层）：平台自营/代理商/城市合伙人是同一概念的三种；Payee 泛化随之上提为两层 | [ADR-029](./ADR/ADR-029-运营主体抽象.md)（2026-09-23 用户定「先进行概念抽象」） |
...
| **自然人** | `agt_principal` | `principal_no` / `phone_hash` · `email_hash`（各自 UNIQUE，做登录查找键）/ `*_mask` 仅显示 / `*_enc` 可逆 / `cred_ref`；⚠️ **掩码列不得进唯一键**——`13800138000` 与 `13811138000` 掩码相同 | **L1**（ADR-030 §2.2） |
| **入驻申请** | `agt_apply` | `apply_no` / `source`(SELF_SERVICE·OPS_CREATED) / `login_phone` / `operator_type` / `status` / `reject_reason` / `operator_no` 回写；「同手机号至多一张在途」用**生成列** `active_key` 而非可空唯一键（MySQL 的 UNIQUE 不约束 NULL） | **L1**（ADR-030 §三，与 OPS-AGT-07 同批） |
