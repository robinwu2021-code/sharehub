-- ============================================================
-- ShareHub · 补上 section 自己的权限码（review 发现，V67 的修正）
--
-- 【V67 漏了什么】
-- V67 把 18 个 section 的 perm 全填了 NULL —— 因为 nav.ts 的 NavSection 上
-- 当时没有 perm 字段。而**旧的 12 行种子其实每个菜单都有入口码**
-- （M_device → device:cabinet:read），是这次迁移把它丢了。
--
-- 【为什么大部分 section 仍然不需要码】
-- 有叶子的 section，可见性就是「有没有可见叶子」：
-- 那既是 sectionDefaultHref 的语义（落到第一个可见叶子），
-- 也避免「L1 显示、点开一片空」（跨模块 section 今天已经这么判了）。
-- 给它再挂一个自己的码，只会多一处要和叶子保持一致的东西。
--
-- 【只有一个 section 需要】
-- 「经营看板」**没有任何叶子**（实测 18 个 section 里只有它是 0）。
-- 按「有可见叶子才显示」判，它会对所有人隐藏 ——
-- 这正是 review 时拦下来的：那条规则写进方案、实现之前先量了一遍数据。
--
-- 整表重灌而不是 UPDATE 一行：数据由 backend/scripts/gen-menu-seed.py 生成，
-- 让「库里的行 = 生成器的输出」保持逐字相等，卡口（ops-web/lib/nav-seed.test.ts）
-- 才比得下去。改成增量 UPDATE 就要在两个地方描述同一份数据。
--
-- type 沿用 V68 钉下的两档 MENU（分组）/ ITEM（叶子），不引入第三套命名。
-- ============================================================
SET NAMES utf8mb4;
