-- ============================================================
-- ShareHub · 13 个菜单叶补上权限码（P1 第二步）
--
-- 【它们今天靠一条隐式规则可见】
-- 无 perm 的叶子「跟随父模块」（前端 canModule）。服务端按码过滤表达不了这条规则 ——
-- 实测不补则每个角色**多出 7–15 项**。这正是 nav.ts 注释里记过的旧伤：
-- 「靠无 perm 的叶子跟随父模块，漏出了 SLA 管理 / 巡检计划 / BD 拓展 CRM」。
--
-- 【码是查出来的，不是猜的】
-- 逐个查该页**列表接口**实际强制的码。第一版我是按命名习惯猜的，13 个错了 3 个：
--   · SLA 管理 / 巡检计划 —— 猜 workorder:{sla,inspection}:update，
--     实际列表判 workorder:wo:read（那两个码只挂在**写**端点上）。
--     我据此提过一个「要不要收窄 VIEWER/BD 可见性」的问题 —— **那个问题不存在**；
--   · 邀请裂变 —— 猜 marketing:referral:read，实际 /api/user/referrals 判
--     marketing:campaign:read（referral:read 存在，但挂在别处）。
--
-- 【可见性变化：104 个组合里变 9 个，全是修复】
--   CS  少 5 项（营销：活动/邀请裂变/广告位/广告活动/投放曝光）
--   OPS 少 4 项（报表：财务/大屏/自定义/消费者）
-- 这 9 项**今天全是「看得见、点不开」**：CS 点「营销›活动」，后端判
-- marketing:campaign:read，CS 没有 → 403；OPS 点「报表›财务」同理。
-- 隐藏它们是修掉一个存在已久的缺陷，不是回归。
-- 逐项记录在 ops-web/lib/nav-visibility.snapshot.txt 的 diff 里。
--
-- 整表重灌而不是 UPDATE 13 行：数据由 gen-menu-seed.py 生成，
-- 让「库里的行 = 生成器输出」逐字相等，卡口 nav-seed.test.ts 才比得下去。
-- ============================================================
SET NAMES utf8mb4;
...
INSERT INTO iam_menu (menu_no, parent_no, name, type, path, icon, group_name, sort, perm, phase, ready, module, modules, match_paths, pin_bottom, portal_for) VALUES
  ...
  ('M_dashboard', NULL, '经营看板', 'MENU', '/', 'LayoutDashboard', NULL, 4, 'dashboard:overview:read', 1, 0, 'dashboard', NULL, NULL, 0, NULL),
  ...
  ('M_workorder__3', 'M_workorder', 'SLA 管理', 'ITEM', '/work-orders?view=sla', NULL, NULL, 3, 'workorder:wo:read', 2, 0, NULL, NULL, NULL, 0, NULL),
  ('M_workorder__4', 'M_workorder', '巡检计划', 'ITEM', '/work-orders?view=inspection', NULL, NULL, 4, 'workorder:wo:read', 2, 0, NULL, NULL, NULL, 0, NULL),
  ...
  ('M_marketing__2', 'M_marketing', '活动', 'ITEM', '/marketing?tab=campaigns', NULL, '促销玩法', 2, 'marketing:campaign:read', 2, 0, NULL, NULL, NULL, 0, NULL),
  ('M_marketing__3', 'M_marketing', '推送触达', 'ITEM', '/marketing?tab=push', NULL, '促销玩法', 3, 'marketing:push:send', 3, 1, NULL, NULL, NULL, 0, NULL),
  ('M_marketing__4', 'M_marketing', '邀请裂变', 'ITEM', '/marketing?tab=referral', NULL, '促销玩法', 4, 'marketing:campaign:read', 3, 0, NULL, NULL, NULL, 0, NULL),
  ('M_marketing__5', 'M_marketing', '广告位管理', 'ITEM', '/marketing?tab=ad-slots', NULL, '广告经营', 5, 'marketing:ad:read', 3, 0, NULL, NULL, NULL, 0, NULL),
  ('M_marketing__6', 'M_marketing', '广告活动', 'ITEM', '/marketing?tab=ad-campaigns', NULL, '广告经营', 6, 'marketing:ad:read', 3, 0, NULL, NULL, NULL, 0, NULL),
  ('M_marketing__7', 'M_marketing', '投放与曝光', 'ITEM', '/marketing?tab=ad-delivery', NULL, '广告经营', 7, 'marketing:ad:read', 3, 0, NULL, NULL, NULL, 0, NULL),
  ('M_cs', NULL, '客服管理', 'MENU', '/cs', 'Headset', NULL, 15, NULL, 1, 0, 'cs', NULL, NULL, 0, NULL),
  ('M_cs__1', 'M_cs', '报障受理', 'ITEM', '/cs', NULL, NULL, 1, 'cs:ticket:read', 1, 0, NULL, NULL, NULL, 0, NULL),
  ('M_cs__2', 'M_cs', '客服会话', 'ITEM', '/cs?tab=sessions', NULL, NULL, 2, 'cs:session:read', 2, 0, NULL, NULL, NULL, 0, NULL),
  ...
  ('M_report__3', 'M_report', '财务报表', 'ITEM', '/reports?tab=finance', NULL, NULL, 3, 'report:finance:read', 2, 0, NULL, NULL, NULL, 0, NULL),
  ('M_report__4', 'M_report', '实时大屏', 'ITEM', '/reports?tab=screen', NULL, NULL, 4, 'report:screen:read', 3, 0, NULL, NULL, NULL, 0, NULL),
  ('M_report__5', 'M_report', '自定义报表', 'ITEM', '/reports?tab=custom', NULL, NULL, 5, 'report:custom:read', 3, 0, NULL, NULL, NULL, 0, NULL),
  ('M_report__6', 'M_report', '消费者分析', 'ITEM', '/reports?tab=consumer', NULL, NULL, 6, 'report:consumer:read', 3, 0, NULL, NULL, NULL, 0, NULL),
